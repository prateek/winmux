...
`make e2e-slice-43` records mouse gesture configurability with `script/e2e/configs/mouse-gesture-configurability.toml`. The proof uses `float-unless-snap` with `gesture = 'secondary-button-drag'` and `target = 'zone'`: an ordinary drag leaves `snap-demo.rtf` floating with no whole-zone overlay, then the same window is reset and dragged with the secondary button held so the product overlay targets the whole Comms zone and the final placement moves to Comms/right.

`make e2e-slice-44` records drag overlay target semantics with `script/e2e/configs/drag-overlay-semantics.toml`. It re-proves the secondary-button whole-zone branch with fresh media, requires the release-boundary screenshot `08-snap-release-slice-44.png`, and adds an explicit no-window-slot contract: the product frame must visibly read `Whole zone: Comms`, while the proof manifest states `slot-noop = 'whole-zone target; no window-slot target active'`. Overlay sentinel crops are supporting evidence only.

`make e2e-slice-45` records deterministic display topology recovery with `script/e2e/configs/column-zones.toml`. The proof is intentionally a Tart simulation, not a real hardware hotplug claim: the topology boards, proof manifest, event manifest, topology event manifest, and topology contact sheet must all state `hardware_hotplug_claim=no` while showing Reference, Work, and Comms remain recoverable through before, simulated loss, recovery, simulated return, topology after, and final recoverability beats.
...
runs `winmux zone init --preset balanced --write` with a backup path and managed
TOML block, validates the generated config, launches `WinMuxApp` without
