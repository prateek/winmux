...
PHASE="${WINMUX_E2E_SLICE35_PHASE:-proof}"
...
UNCOMMENTED_CONFIG="${ARTIFACTS_DIR}/logs/slice-35-starter-config-uncommented.toml"
GUEST_DISPLAY_ID="${GUEST_DISPLAY_ID:-1}"
RECORDING_NAME="slice-35-starter-ultrawide-template"

APP_LOG="${ARTIFACTS_DIR}/logs/winmux-app.log"
APP_LOG_LOCAL="/tmp/winmux-e2e-slice35-app.log"
STARTUP_TRACE="${ARTIFACTS_DIR}/logs/winmux-startup-trace.log"
STARTUP_TRACE_LOCAL="/tmp/winmux-e2e-slice35-startup.log"
LAUNCH_STATUS="${ARTIFACTS_DIR}/logs/winmux-launchagent-status.log"
LAUNCH_LABEL="local.winmux.e2e.slice35"
LAUNCH_PLIST="/tmp/winmux-e2e-slice35.plist"
LAUNCH_PLIST_COPY="${ARTIFACTS_DIR}/logs/winmux-e2e-slice35.plist"

SETUP_LOG="${ARTIFACTS_DIR}/logs/slice-35-setup.log"
CONFIG_BEFORE_COPY="${ARTIFACTS_DIR}/logs/slice-35-starter-config-before.toml"
CONFIG_CHECK_LOG="${ARTIFACTS_DIR}/logs/slice-35-config-check.log"
LIST_ZONES_LOG="${ARTIFACTS_DIR}/logs/slice-35-list-zones.log"
CLI_LOG="${ARTIFACTS_DIR}/logs/slice-35-cli.log"
TIMING_LOG="${ARTIFACTS_DIR}/logs/slice-35-command-timing.log"
VISIBLE_UNCOMMENTED_TEMPLATE="${ARTIFACTS_DIR}/logs/slice-35-visible-uncommented-template.txt"
WAIT_ERR="${ARTIFACTS_DIR}/logs/slice-35-cli-wait.err"
DONE="${ARTIFACTS_DIR}/logs/${RECORDING_NAME}.done"
PROOF="${ARTIFACTS_DIR}/slice-35-starter-ultrawide-template-proof.txt"
...
write_result_doc() {
    {
        printf 'Slice 35: starter ultrawide template\n'
        printf '\n'
        ...
        cat "${LIST_ZONES_LOG}"
        printf '\n'
        printf 'Template markers remain in the source config so users can find the opt-in block.\n'
    } >"${ARTIFACTS_DIR}/logs/slice-35-visible-result.txt"
    write_text_doc "${ARTIFACTS_DIR}/logs/slice-35-visible-result.txt" \
...
PLIST
    ...
    for _ in $(seq 1 60); do
        ...
        copy_runtime_logs
        if "${CLI}" list-zones --count >"${ARTIFACTS_DIR}/logs/slice-35-zone-count.txt" 2>"${WAIT_ERR}"; then
            return
        fi
        sleep 1
    done
...
setup_slice() {
    rm -f \
        ...
        "${LIST_ZONES_LOG}" "${CLI_LOG}" "${TIMING_LOG}" "${WAIT_ERR}" "${DONE}" "${PROOF}" \
    ...
    ' "${CONFIG_BEFORE_COPY}" >"${ARTIFACTS_DIR}/logs/slice-35-visible-commented-template.txt"
    write_text_doc "${ARTIFACTS_DIR}/logs/slice-35-visible-commented-template.txt" \
    ...
    {
        echo 'WinMux Slice 35: starter ultrawide template'
        echo "Starter config: ${STARTER_CONFIG}"
        echo "Template marker: WINMUX ULTRAWIDE ZONES TEMPLATE"
        echo 'Commands: uncomment template; winmux config --check; winmux list-zones'
...
proof_slice() {
    ...
    sleep 3
    capture_guest_screenshot '02-template-uncommented-slice-35'
    sleep 6
    ...
    sleep 8
    capture_guest_screenshot '03-config-check-and-zones-slice-35'
    sleep 12

    cat >"${PROOF}" <<PROOF_TEXT
PASS: starter config keeps zones commented by default, the WINMUX ULTRAWIDE ZONES TEMPLATE uncomments into parseable TOML, and WinMux lists Reference, Work, and Comms zones after launch with the uncommented config.
...
case "${PHASE}" in
    ...
    self-test)
        ...
        ;;
    *)
        echo "Unknown Slice 35 phase: ${PHASE}" >&2
        exit 64
        ;;
esac
