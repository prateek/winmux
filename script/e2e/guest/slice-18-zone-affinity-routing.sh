...
PHASE="${WINMUX_E2E_SLICE18_PHASE:-proof}"
...
APP_LOG_LOCAL="/tmp/winmux-e2e-slice18-app.log"
STARTUP_TRACE="${ARTIFACTS_DIR}/logs/winmux-startup-trace.log"
STARTUP_TRACE_LOCAL="/tmp/winmux-e2e-slice18-startup.log"
LAUNCH_STATUS="${ARTIFACTS_DIR}/logs/winmux-launchagent-status.log"
LAUNCH_LABEL="local.winmux.e2e.slice18"
LAUNCH_PLIST="/tmp/winmux-e2e-slice18.plist"
LAUNCH_PLIST_COPY="${ARTIFACTS_DIR}/logs/winmux-e2e-slice18.plist"

SETUP_LOG="${ARTIFACTS_DIR}/logs/slice-18-zone-affinity-routing-setup.log"
WINDOW_SETUP_LOG="${ARTIFACTS_DIR}/logs/slice-18-affinity-setup-windows.log"
WINDOW_BEFORE_LOG="${ARTIFACTS_DIR}/logs/slice-18-affinity-before.log"
OPEN_LOG="${ARTIFACTS_DIR}/logs/slice-18-open-affinity-window.log"
WINDOW_AFTER_LOG="${ARTIFACTS_DIR}/logs/slice-18-affinity-after.log"
ZONES_LOG="${ARTIFACTS_DIR}/logs/slice-18-zones.log"
CLI_LOG="${ARTIFACTS_DIR}/logs/slice-18-cli.log"
WAIT_ERR="${ARTIFACTS_DIR}/logs/slice-18-cli-wait.err"
STATE_FILE="${ARTIFACTS_DIR}/logs/slice-18-zone-affinity-state.env"
DONE="${ARTIFACTS_DIR}/logs/slice-18-zone-affinity-routing.done"
PROOF="${ARTIFACTS_DIR}/slice-18-zone-affinity-routing-proof.txt"

DOC_DIR="${HOME}/winmux-e2e/zone-affinity-docs"
REFERENCE_DOC="${DOC_DIR}/reference-anchor.rtf"
WORK_DOC="${DOC_DIR}/focused-work.rtf"
COMMS_DOC="${DOC_DIR}/comms-anchor.rtf"
AFFINITY_DOC="${DOC_DIR}/affinity-comms.rtf"
...
write_doc() {
    ...
    local detail="$4"
    cat >"${path}" <<RTF
{\rtf1\ansi\deff0{\fonttbl{\f0 Helvetica;}}\viewkind4\uc1\pard\qc\f0\fs104 ${headline}\par\fs56 ${subtitle}\par\fs34 ${detail}\par}
RTF
}
...
PLIST
    ...
    for _ in $(seq 1 60); do
        ...
        copy_runtime_logs
        if "${CLI}" list-zones --count >"${ARTIFACTS_DIR}/logs/slice-18-zone-count.txt" 2>"${WAIT_ERR}"; then
            return
        fi
        sleep 1
    done
...
setup_slice() {
    ...
    {
        echo 'WinMux Slice 18: zone affinities route matching windows'
        echo "Source App: ${SOURCE_APP}"
        echo "Source CLI: ${SOURCE_CLI}"
        echo "Config: ${CONFIG}"
        echo "Config: [[zone-affinities]] zone = 'Comms', if.window-title-regex-substring = 'affinity-comms'"
        echo 'Non-claim: no relaunch persistence or durable tab-group identity'
    } | tee "${SETUP_LOG}"
    ...
    write_doc "${REFERENCE_DOC}" 'REFERENCE' 'Anchor window' 'left zone before affinity proof'
    write_doc "${WORK_DOC}" 'FOCUSED WORK' 'Window that must stay in Work' 'the affinity routes the detected window, not this one'
    write_doc "${COMMS_DOC}" 'COMMS' 'Destination anchor' 'right zone before affinity proof'
    write_doc "${AFFINITY_DOC}" 'AFFINITY COMMS' 'Matches [[zone-affinities]]' 'affinity-comms should move to Comms'
    ...
    WORK_ID="$(window_id_for_title "${WINDOW_SETUP_LOG}" 'focused-work.rtf')"
    ...
    setup_window_zone "${WORK_ID}" 'focused-work.rtf' Work main
    ...
    assert_window_zone "${WINDOW_BEFORE_LOG}" 'focused-work.rtf' main
    assert_window_zone "${WINDOW_BEFORE_LOG}" 'comms-anchor.rtf' right
    assert_title_absent "${WINDOW_BEFORE_LOG}" 'affinity-comms.rtf'

    cat >"${STATE_FILE}" <<STATE
WORK_ID=${WORK_ID}
AFFINITY_DOC=${AFFINITY_DOC}
STATE

    {
        echo 'setup=result=success'
        echo 'ready-state=reference-work-comms-visible'
        echo "focused-work-window-id=${WORK_ID}"
        echo "affinity-doc=${AFFINITY_DOC}"
        echo 'ready-screenshot-label=setup checkpoint; recording-time proof starts after capture begins'
    } | tee -a "${SETUP_LOG}"
    copy_runtime_logs
}
...
proof_slice() {
    ...
    {
        echo '$ open -a TextEdit affinity-comms.rtf'
        /usr/bin/open -a TextEdit "${AFFINITY_DOC}"
    } | tee "${OPEN_LOG}"

    for _ in $(seq 1 60); do
        refresh_window_log "${WINDOW_AFTER_LOG}"
        if [ "$(zone_for_title "${WINDOW_AFTER_LOG}" 'affinity-comms.rtf')" = right ]; then
            break
        fi
        sleep 1
    done

    AFFINITY_ID="$(window_id_for_title "${WINDOW_AFTER_LOG}" 'affinity-comms.rtf')"
    AFFINITY_ZONE="$(zone_for_title "${WINDOW_AFTER_LOG}" 'affinity-comms.rtf')"
    AFFINITY_WORKSPACE="$(workspace_for_title "${WINDOW_AFTER_LOG}" 'affinity-comms.rtf')"
    WORK_ZONE_AFTER="$(zone_for_title "${WINDOW_AFTER_LOG}" 'focused-work.rtf')"
    WORK_WORKSPACE_AFTER="$(workspace_for_title "${WINDOW_AFTER_LOG}" 'focused-work.rtf')"
    [ -n "${AFFINITY_ID}" ] || semantic_fail 'affinity-comms.rtf did not appear in visible windows'
    [ "${AFFINITY_ZONE}" = right ] || semantic_fail "affinity-comms.rtf did not route to Comms/right; got ${AFFINITY_ZONE:-missing}"
    [ -n "${AFFINITY_WORKSPACE}" ] || semantic_fail 'affinity-comms.rtf missing workspace after routing'
    [ "${WORK_ZONE_AFTER}" = main ] || semantic_fail "focused-work.rtf moved unexpectedly; zone=${WORK_ZONE_AFTER:-missing}"
    [ -n "${WORK_WORKSPACE_AFTER}" ] || semantic_fail 'focused-work.rtf missing workspace after routing'
...

    {
        echo 'WinMux Slice 18: zone affinities route matching windows'
        echo
        echo 'Configured affinity:'
        echo "[[zone-affinities]] zone = 'Comms'"
        echo "if.window-title-regex-substring = 'affinity-comms'"
        echo
        echo 'Windows before opening affinity document:'
        cat "${WINDOW_BEFORE_LOG}"
        echo
        echo 'User action:'
        cat "${OPEN_LOG}"
        echo
        echo 'Windows after affinity routing:'
        ...
        echo
        echo "focused-work-window-id=${WORK_ID}"
        echo "focused-work-zone-after=${WORK_ZONE_AFTER}"
        echo "routed-window-id=${AFFINITY_ID}"
        echo "routed-zone=${AFFINITY_ZONE}"
        echo "routed-workspace=${AFFINITY_WORKSPACE}"
        echo 'non-claim=no relaunch persistence or durable tab-group identity'
        echo
        echo 'PASS: zone-affinities routed affinity-comms.rtf to the Comms zone using the detected window id while focused-work.rtf stayed in Work.'
    } >"${PROOF}"
...
case "${PHASE}" in
    ...
    proof)
        ...
        ;;
    *)
        echo "Unknown WINMUX_E2E_SLICE18_PHASE: ${PHASE}" >&2
        exit 64
        ;;
esac
