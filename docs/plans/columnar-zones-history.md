# Columnar Zones — Slice History

Historical per-slice record for the columnar-zones effort: goals, required
scope, and the accepted Tart acceptance evidence for each slice as it
shipped. This is an archive kept for provenance and archaeology; the
forward plan and design live in `columnar-zones.md`. Artifact paths below
reference the untracked `artifacts/e2e/` evidence tree from the build
machine at acceptance time and may no longer exist locally.
